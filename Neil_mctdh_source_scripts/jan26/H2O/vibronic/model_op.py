...
def extract_normal_mode_frequencies(path, memmap):
    ...
    try:
        ...
        print(f"Couldn't find {beginString}")
        return
...
def extract_string_list(path, memmap, header_index=2):
    ...
    return [line.split() for line in stringData.splitlines() if "#" not in line and line is not ""]


def extract_transition_dipole_moments(path, memmap, dipole_moments):
    ...
    """Need to figure out why dipole moments in .op files have dimensionality A-1
    which is different from the models provided to me by Songhao
    """
    for a in range(len(lines)):
        len_2_list = lines[a]
        assert len_2_list[0] == f"Ex_s00_s{a+1:02}"
        dipole_moments[a] = complex(len_2_list[1])
    return
...
def mode_symmetrize_quadratic_terms(number_of_modes, States, quadratic_terms):
    ...
    for a, b in it.product(States, States):
        ...
        for a, b in it.product(States, States):
            quadratic_terms[(*diagonal_idx, a, b)] *= 2.0
...
def read_model_op_file(
    ...
    with open(path_file_op, "r+b") as source_file:
        ...
        with mmap.mmap(source_file.fileno(), 0, **read_write_prot_kwarg) as mm:
            ...
            dipole_moments = np.zeros(size['A'], dtype=C128)
            ...
            if get_transition_dipole_moment:
                extract_transition_dipole_moments(path_file_op, mm, dipole_moments)
    ...
    if double_quadratic:
        ...
        print("We didn't change the quadratic terms AT ALL, make sure this agrees with the PIMC")
...
def build_electronic_moments(model):
    """A placeholder that does nothing for now."""
    return ''.join([
        make_line(label=f"Ex_s00_s{a+1:0>2d}", value=float(model[VMK.tdm][a]))
...
def build_parameter_section(diagonal_model, flag_diagonal=False):
...
def build_hamiltonian_section(model, flag_diagonal=False):
...
def generate_op_file_data(model, flag_diagonal=False):
    ...
    for key in VMK.coupling_list():
        if key not in model.keys():
            if flag_diagonal:
                model[key] = diagonal_model_zeros_template_json_dict(model[VMK.A], model[VMK.N])[key]
            else:
                model[key] = model_zeros_template_json_dict(model[VMK.A], model[VMK.N])[key]

    return '\n\n\n'.join([
        build_op_section(),
        build_parameter_section(model, flag_diagonal),
        build_hamiltonian_section(model, flag_diagonal),
