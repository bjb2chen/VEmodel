...
def diabatization(**kwargs):
    ...
    for i in range(N):
        ...
        if pp.skip_bilin_terms:
            continue  # skip the bilinear code below

        # 2D distortion to get bilinear vibronic coupling
        for j in range(0, i):

            if (i, j) not in special_bilin_terms:  # skip all the other bilin terms
                continue
...
def mctdh(op_path, hessian_path, all_frequencies_cm, A, N, **kwargs):
    ...
    def make_parameter_section(model, A, N):
        ...
        key = 'BiLinear'
        if key in model.keys() and not pp.skip_bilin_terms:
    ...
    def _write_op():
        ...
        print(model['BiLinear']); breakpoint();
    ...
    def confirm_necessary_files_exist():
        ...
        def _confirm_linear_are_good():
            ...
            for i in range(N):
                ...
                if not all(code == 0 for code in grace_code.values()):
                    ...
                    bad_mode_list.append(i)

            if len(bad_modes) == 0:
                return True, []
            else:
                return False, bad_mode_list

        def _confirm_bilinear_are_good():
            """ confirm all displacement_input_files are good for all selected modes """
            bad_mode_list = []
            for i, j in upper_triangle_loop_indices(N, 2):
                grace_code = {}
        ...
        b2, bad_list = _confirm_linear_are_good()
        # breakpoint(); print(f"These modes were bad {bad_list=})
        # flag |= b2

        if pp.skip_bilin_terms:
            b3, bad_list = True, []
        else:
            b3, bad_list = _confirm_bilinear_are_good()
            # breakpoint(); print(f"These modes were bad {bad_list=})

        flag = bool(b1 and b2 and b3)
